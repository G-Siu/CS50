...
RACES = ["Dragonborn", "Dwarf", "Elf", "Gnome", "Half-Elf", "Halfing", "Half-Orc", "Human", "Tiefling"]
        ...
        try:
            ...
            if ask == "y" or ask == "yes":
                ...
                if roll == 20:
                    ...
                    # Get modifier from ability score and add to the random roll
                    modifier = modifiers("Choose an ability score modifier: STR | DEX | CON | INT | WIS | CHA", char_score)
                    total_roll = roll + modifier
                    print(f"You rolled {roll}. With a of {modifier}, your total roll is {total_roll}!")
                    continue
            else:
...
def calculate_scores(char_race):
    ...
    if char_race == "Half-Elf":
        while True:
            try:
                add_score = input("Add 1 to two ability scores. Type two with space between from STR | DEX | CON | INT | WIS\n").upper().split()
    ...
    for _ in range(len(ABILITY_SCORES)):
        print(stats)
        choose_stat = input(f"For a roll of {stats[i]}, choose to assign to an ability score STR | DEX | CON | INT | WIS | CHA\n")
...
# Determine type of roll, and randomise a number returned based on chosen die faces
def dice():
    while True:
        try:
            roll_type = input("Advantage: 1 | Disadvantage: 2 | Straight: 3? ")
            roll_type = roll_type.lower().strip()
            if roll_type == "advantage" or roll_type.strip() == "1" or roll_type == "disadvantage" or roll_type.strip() =="2":
                die_count = 2
                break
